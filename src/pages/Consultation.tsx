...
export default function Consultation() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // no-cors gives an opaque response, so we can't read success/failure
      // from the server — a completed request without a network error is
        ...
        <div className="max-w-3xl mx-auto px-6 lg:px-8">
          {step === 3 ? (
            ...
            >
              ...
              <p className="text-base mb-2" style={{ color: 'var(--color-charcoal)' }}>
                We've sent a confirmation to <strong>{form.email}</strong> for your requested slot of{' '}
                <strong>{form.date}</strong> at <strong>{form.time}</strong>.
              </p>
              <p className="text-sm mb-8" style={{ color: 'var(--color-muted)' }}>
                Our team will confirm the slot and share joining details shortly.
              </p>
              <div
                className="inline-block px-6 py-4 border rounded-lg text-sm"
                style={{ color: 'var(--color-charcoal)', borderColor: 'var(--color-border)', background: 'var(--color-surface)' }}
              >
                <p className="font-semibold mb-1" style={{ color: 'var(--color-primary)' }}>Next steps</p>
                <p>Check your inbox (and spam folder) for our confirmation email.</p>
              </div>
            </div>
          ) : (
            <form
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="flex flex-col sm:flex-row gap-3">
                      <button
                        ...
                        onClick={() => setStep(1)}
                        disabled={submitting}
                        className="px-6 py-4 text-sm font-medium border rounded transition-colors cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed"
                        style={{ color: 'var(--color-charcoal)', borderColor: 'var(--color-border)' }}
                      >
                        Back
                      </button>
